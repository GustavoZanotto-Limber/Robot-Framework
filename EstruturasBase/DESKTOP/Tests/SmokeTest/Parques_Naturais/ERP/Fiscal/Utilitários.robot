...
Informações > Obrigações do PIS_Cofins a recolher
...
Outros > Outras Bases > IRPJ > Outras Deduções do IRF p_ IRPJ - IRPJ
...
Outros > PIS/COFINS > Lançamento de Crédito do Pis_Cofins
...
Outros > PIS/COFINS > Utilização de Crédito de Pis_Cofins
...
Outros > Exportações > Saldo de NF's p_ Exportação
    ${nome_print}=     Set Variable     Saldo de NF's p_ Exportação
    [Tags]    Fiscal   Movimentos   SmokeTest
    Movimentos                Outros
    RPA.Windows.Click         Exportações
    RPA.Desktop.Press Keys    S
    ...
    RPA.Desktop.Press Keys    0
    RPA.Desktop.Press Keys    Enter
    BaseDesktop.Screenshot    Consulta de Saldo de NF's para Exportação (1)          ${Caminho_Screenshots}Outros/Exportações/Saldo de NF's p_ Exportação
    RPA.Windows.Click         Confirmar
    Fechar janela
